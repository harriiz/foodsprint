  ...
  const handleSearchClick = () => {
    ...
    for (let i = 0; i < restorani.length; i++) {
      if (searchInput.toLowerCase() === restorani[i].naziv.toLowerCase()) {
        navigate(`/restoraniedit/${restorani[i].id}`);
        matchFound = true;
        console.log(
      ...
      <div className="searchNavigacija">
        <div className="inputContainerMain">
          <input
            ...
            id=""
            className="mainInput"
            placeholder="Pronađite restoran koji želite urediti"
            ...
            >
              {suggestions.map((suggestion) => (
                <div
                  ...
                  key={suggestion.id}
                  onClick={() => navigate(`/restoraniedit/${suggestion.id}`)}
                >
                  {suggestion.naziv}
                </div>
              ))}
